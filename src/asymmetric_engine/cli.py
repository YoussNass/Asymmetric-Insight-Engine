...
from asymmetric_engine.application.evidence_ingestion import IngestSourceDocument, ListSourceDocumentsAt
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    evidence = subparsers.add_parser("evidence", help="Operate the local append-only source-evidence ledger.")
    evidence_commands = evidence.add_subparsers(dest="evidence_command", required=True)
    ingest_sec = evidence_commands.add_parser("ingest-sec", help="Ingest exact SEC CIK/accession references sequentially.")
    ...
    list_documents = evidence_commands.add_parser("list", help="List source versions knowable at a decision-time boundary.")
    _add_knowledge_boundary_arguments(list_documents)
    coverage = evidence_commands.add_parser("coverage", help="Report inclusion, exclusion, and temporal-provenance counts.")
    _add_knowledge_boundary_arguments(coverage)
    verify = evidence_commands.add_parser("verify", help="Recompute one stored source document's hash and byte size.")
    verify.add_argument("--database", type=Path, required=True)
    verify.add_argument("--document-id", type=UUID, required=True)

    workspace = subparsers.add_parser("workspace", help="Open the local read-only operator workspace.")
...
def _add_knowledge_boundary_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--knowledge-mode", choices=tuple(mode.value for mode in KnowledgeMode), required=True)


def doctor_payload() -> dict[str, str]:
    return {"package": "asymmetric-insight-engine", "python": platform.python_version(), "status": "ok", "version": __version__}
...
def _knowledge_boundary(args: argparse.Namespace) -> KnowledgeBoundary:
    return KnowledgeBoundary(as_of=args.as_of, knowledge_mode=KnowledgeMode(args.knowledge_mode))
...
def _run_ingest_sec(args: argparse.Namespace) -> int:
    ...
    except (ValueError, InvalidBatchInputError) as error:
    ...
    result = IngestSourceDocuments(
        IngestSourceDocument(provider=SecEdgarProvider(fetcher.fetch), repository=repository, clock=SystemClock())
    ).execute(references)
    outcomes: list[dict[str, Any]] = []
    for outcome in result.outcomes:
        if outcome.append_result is not None:
            outcomes.append({"document": _document_payload(outcome.append_result.document), "reference": outcome.reference, "status": outcome.append_result.status.value})
        else:
            outcomes.append({"failure_kind": outcome.failure_kind.value if outcome.failure_kind is not None else None, "failure_message": outcome.failure_message, "reference": outcome.reference, "status": "failed"})
    print(json.dumps({"command": "evidence.ingest_sec", "failed": result.failed_count, "outcomes": outcomes, "succeeded": result.succeeded_count}, sort_keys=True))
...
def _run_list_documents(args: argparse.Namespace) -> int:
    ...
    documents = ListSourceDocumentsAt(_existing_repository(args.database)).execute(subject_id=subject_id, boundary=boundary)
    print(json.dumps({"as_of": boundary.as_of.isoformat(), "documents": [_document_payload(document) for document in documents], "knowledge_mode": boundary.knowledge_mode.value, "subject_id": subject_id}, sort_keys=True))
    return 0
...
def _run_coverage(args: argparse.Namespace) -> int:
    boundary = _knowledge_boundary(args)
    subject_id = _required_text(args.subject, "subject")
    report = InspectKnowledgeCoverage(_existing_repository(args.database)).execute(subject_id=subject_id, boundary=boundary)
    print(json.dumps({"as_of": report.boundary.as_of.isoformat(), "included_versions": report.included_versions, "knowledge_mode": report.boundary.knowledge_mode.value, "observed_at_ingestion_versions": report.observed_at_ingestion_versions, "provider_asserted_versions": report.provider_asserted_versions, "record_not_ingested": report.record_not_ingested, "source_not_available": report.source_not_available, "subject_id": report.subject_id, "total_versions": report.total_versions, "warnings": report.warnings}, sort_keys=True))
    return 0


def _run_verify(args: argparse.Namespace) -> int:
    report = VerifySourceDocument(_existing_repository(args.database)).execute(args.document_id)
    print(json.dumps({"actual_content_hash": report.actual_content_hash, "actual_content_size_bytes": report.actual_content_size_bytes, "document_id": str(report.document.document_id), "expected_content_hash": report.document.content_hash, "expected_content_size_bytes": report.document.content_size_bytes, "hash_matches": report.hash_matches, "size_matches": report.size_matches, "status": "ok" if report.is_valid else "corrupt"}, sort_keys=True))
    return 0 if report.is_valid else 3


def _run_workspace(args: argparse.Namespace) -> int:
    try:
        repository = SQLiteProductRecordRepository(args.database, initialize_schema=False)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "workspace":
        try:
            ...
            print(json.dumps({"error": type(error).__name__, "message": str(error), "status": "error"}, sort_keys=True))
            return 2
