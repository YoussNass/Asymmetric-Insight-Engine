...
- tampered rows are detected independently of Pydantic shape validation;
...
7. corruption remains blocking even if database guards are bypassed;
8. database and record-schema mismatches fail closed and require explicit migration;
9. read-only opening of a missing store cannot create it as a side effect;
10. documentation states that loaded decision records still require canonical application replay;
11. no production database, ORM, HTTP server, broker, financial score, sizing, timing, or Learning feedback enters the slice;
12. lint, formatting, mypy, pytest, package, Python 3.12/3.13 and container CI pass on the exact PR head;
13. the owner explicitly accepts ADR 0021. Merge authorization remains a separate governance gate.
