...
def test_verified_capital_decisions_flow_through_execution_into_learning() -> None:
    ...
    assert (
        allocation_case.source.decision_boundary.as_of <= allocation_case.knowledge_boundary.as_of
    )
    assert (
        allocation_case.knowledge_boundary.as_of <= allocation_evaluation.knowledge_boundary.as_of
    )
    ...
    replacement_context, _, replacement_plan, _, replacement_case = make_replacement_learning_case()
